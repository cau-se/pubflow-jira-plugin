...
import de.pubflow.components.jira.wsArtifacts.JiraEndpoint;
import de.pubflow.components.jira.wsArtifacts.JiraEndpointService;
...
public class JiraPluginMsgConsumer {
	...
	private void createIssueType(HashMap<String, String> map) throws Exception{
		de.pubflow.components.jira.wsArtifacts.CreateIssueType.Arg2 params = 
				new de.pubflow.components.jira.wsArtifacts.CreateIssueType.Arg2();

		for(Entry<String, String> entry : CamelJiraMessage.getMap("parameters", map).entrySet()){
			de.pubflow.components.jira.wsArtifacts.CreateIssueType.Arg2.Entry newEntry = 
					new de.pubflow.components.jira.wsArtifacts.CreateIssueType.Arg2.Entry();
	...
	private void createIssue(HashMap<String, String> map) throws Exception{
		de.pubflow.components.jira.wsArtifacts.CreateIssue.Arg4 params = 
				new de.pubflow.components.jira.wsArtifacts.CreateIssue.Arg4();
		for(Entry<String, String> entry : CamelJiraMessage.getMap("parameters", map).entrySet()){
			de.pubflow.components.jira.wsArtifacts.CreateIssue.Arg4.Entry newEntry = 
					new de.pubflow.components.jira.wsArtifacts.CreateIssue.Arg4.Entry();
			newEntry.setKey(entry.getKey());
			newEntry.setValue(entry.getValue());
			params.getEntry().add(newEntry);
		}
