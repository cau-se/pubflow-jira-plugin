...
import de.pubflow.jira.wsArtifacts.IPubFlowToJiraConnector;
import de.pubflow.jira.wsArtifacts.PubFlowToJiraConnectorService;
...
public class JiraPluginMsgConsumer {
	...
	private void createIssueType(HashMap<String, String> map){
		de.pubflow.jira.wsArtifacts.CreateIssueType.Params params = 
				new de.pubflow.jira.wsArtifacts.CreateIssueType.Params();
		
		for(Entry<String, String> entry : PubFlowMessage.getMap("parameters", map).entrySet()){
			de.pubflow.jira.wsArtifacts.CreateIssueType.Params.Entry newEntry = 
					new de.pubflow.jira.wsArtifacts.CreateIssueType.Params.Entry();
	...
	private void createIssue(HashMap<String, String> map){
		de.pubflow.jira.wsArtifacts.CreateIssue.Params params = 
				new de.pubflow.jira.wsArtifacts.CreateIssue.Params();
		
		for(Entry<String, String> entry : PubFlowMessage.getMap("parameters", map).entrySet()){
			de.pubflow.jira.wsArtifacts.CreateIssue.Params.Entry newEntry = 
					new de.pubflow.jira.wsArtifacts.CreateIssue.Params.Entry();
			newEntry.setKey(entry.getKey());
			newEntry.setValue(entry.getValue());
			params.getEntry().add(newEntry);
		}
